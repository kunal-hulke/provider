...
const PhotographerFormPage = () => {
  ...
  const photographyTypeOptions = [
    'Candid', 'Traditional', 'Pre-wedding', 'Post-wedding', 'Drone Photography'
  ];
  ...
  const addPhotographyType = () => {
    setFormData(prev => ({
      ...prev,
      photographyTypes: [...prev.photographyTypes, {
        phtype: '',
        pricePerEvent: '',
        sampleWork: []
      }]
  ...
  const handleSampleWorkUpload = (typeIndex, files) => {
    if (files && files.length > 0) {
      ...
      updatedTypes[typeIndex].sampleWork = [...updatedTypes[typeIndex].sampleWork, ...newImages];
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    toast.success(isEditing ? 'Photographer updated successfully!' : 'Photographer added successfully!');
    navigate('/vendors');
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {formData.photographyTypes.map((type, typeIndex) => (
                <div key={typeIndex} className="bg-blue-50 p-4 rounded-lg mb-4 border">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="font-medium">Photography Type #{typeIndex + 1}</h4>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                    <Select
                      ...
                      options={[
                        ...
                        ...photographyTypeOptions.map(option => ({ value: option, label: option }))
                      ]}
                      value={type.phtype}
                      onChange={(value) => updatePhotographyType(typeIndex, 'phtype', value)}
                      fullWidth
                    />
                    ...
                    <Input
                      ...
                      onChange={(e) => updatePhotographyType(typeIndex, 'pricePerEvent', e.target.value)}
                      placeholder="₹ 0"
                      fullWidth
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center mb-4">
                      ...
                      <p className="mt-2 text-sm text-gray-600">Upload sample work (Multiple images allowed)</p>
                      <input 
                        type="file" 
                        multiple 
                        accept="image/*" 
                        ...
                        onChange={(e) => handleSampleWorkUpload(typeIndex, e.target.files)}
                      />
                      <label 
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        {type.sampleWork.map((image, imageIndex) => (
                          <div key={imageIndex} className="relative">
                            ...
                            <button
                              type="button"
                              onClick={() => removeSampleWork(typeIndex, imageIndex)}
